...
void PathPlanner::calc_velocity(const int& particle_id) // returns velocity
{
	...
	auto unit_group = map_groups[group_id];
	...
	if (IS_FORMATION)
	{
		...
		{
			// Virtual Pointer 갱신
			p->goal = unit_group->end;
			unit_group->update_short_range_goal();
		}
		else
		{
			p->goal = p->is_link ? p->offset : unit_group->leader->goal;
		}
	}
	...
	{
		// 목적지에서 관성 줄이기
		p->V_pref = length < V_PREF_ACCEL ? length : V_PREF_ACCEL;

		this->velocity_buffer[particle_id] /= length;  // normalize
		this->velocity_buffer[particle_id] *= p->V_pref;
	}
}