...
void Simulation::do_time_step_force() {
	...
	for (int i = 0; i < 1; i++) {
		...
		for (int i = 0; i < num_particles; i++) {
			...
			particles[i]->Accel.z =
				(particles[i]->V_prev.z - particles[i]->V.z) / KSI_ACCEL;
...
void Simulation::do_time_step()
{
	...
	for (int i = 0; i < num_particles; i++)
	{
		particles[i]->Delta_x = glm::vec3(0, 0, 0);
		particles[i]->Delta_x_ctr = 0;
	}

	// 4. long_range constraint (4.4, 4.5)
	project_longrange_constraints();

	// 5. Short range Constraint
	...
	for (int i = 0; i < num_particles; i++)
	{
		...
		/*if (FTL)
		{
			...
			if (index_next < num_particles)
			{
				if (particles[index_next]->group_id == particles[i]->group_id)
				{
					glm::vec3 d_next = glm::vec3(particles[index_next]->Delta_x.x, particles[index_next]->Delta_x.z, particles[index_next]->Delta_x.z);
					float s_damping = 0.8f;

					ftl = (d_next * s_damping) / time_step;
		...
		if (COHESION)
		{
			...
			for (int j = 0; j < num_particles; j++)
			{
				if (particles[i]->group_id == particles[j]->group_id)
				{
					...
					float margin = particles[i]->r + particles[j]->r;
		...
		particles[i]->X.z = particles[i]->X_pred.z;
	}
