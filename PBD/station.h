...
static std::vector<int> a_star(std::vector<Station> stations, int start, int dest)
{
	...
	{
		// 현재 노드에 연결되어있는 노드들과 그 노드까지의 거리
		std::map<int, float> Stations_linked = clone[current_id].stations_linked;

		// (in while) 2. 출발노드부터 연결된 노드들의 정보를 갱신하면서 가장 작은 f값을 Open 리스트에 삽입
		for (auto iter : Stations_linked)
		{
			// linked Station id
			int id = iter.first;

			// ptr : 노드 자체를 수정하기 위해 포인터로 받음
			...
			{
				// close에 들어가있는 항목이 아니라면 최신화
				if (close.count(Station_linked->id) == 0)
				{
					Station_linked->update(g, h, current_id);
				}

				// Open List에도 없고 Close List에도 없으면 추가 (Open에 잇어야 Close에도 있을 수 있음)
		...
		// open list 를 f 값 기준으로 정렬 후 가장 앞에 값을 close list로 넣기 위한 임시 변수
		std::vector<std::pair<int, Station>> dump = std::vector<std::pair<int, Station>>();

		// (in while) 3. open list에 있는 항목 중에 가장 가깝다고 판단하는 값을 close에 삽입
		if (open.size() > 0)
		{
			// 맵 정렬 방법 : map -> vector -> sort (require cmp function)
