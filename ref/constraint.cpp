...
void Powerlaw_Constraint::project(Particle** particles)
{
	...
	if (d_sq > 0 && (a < -_EPSILON || a > _EPSILON))
	{
		...
		if (tao > 0 && tao < tao0)
		{
			...
			// pick the min solution that is > 0 (   ->    浹Ѵ)
			...
			if (constraint_value < 0)
			{
				...
				if (AVOIDANCE)
				{
					glm::vec3 modified_i = dia_i + delta_X[0];  // long range collision ؼҵ ġ
					glm::vec3 modified_j = dia_j + delta_X[1];  // margin (5%  2   ū  )
...
const float Powerlaw_Constraint::tao0 = 10.00;
...
void Wall_Constraint::project(Particle** particles)
{
	...
	// intersection (line & Rect  ִ 2)
	glm::vec3 two_points[2] = {
		glm::vec3(INFINITE, 0, INFINITE),
		glm::vec3(INFINITE, 0, INFINITE) };
	glm::vec3 intersection = glm::vec3(0, 0, 0);  //  2   ġ 
	...
	if (intersection.x != INFINITE && intersection.z != INFINITE)
	{
		...
		glm::vec3 penetrate = intersection - look_ahead;  // wall İ vector
	...
	//		float2 penetrate = intersection - look_ahead;  // wall İ vector
	//		float penetrate_length = norm(penetrate);
